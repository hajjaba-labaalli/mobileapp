...
import com.androiddevs.labaalliPostsApp.utilities.SEARCH_POST_TIME_DELAY
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        etSearch.addTextChangedListener { editable ->
            ...
            job = MainScope().launch {
                delay(SEARCH_POST_TIME_DELAY)
                editable?.let {
                    if(editable.toString().isNotEmpty()) {
                        viewModel.searchPost(editable.toString())
                    }
                }
            }
        }
        viewModel.searchPost.observe(viewLifecycleOwner, Observer {   response->
            when(response){
                is Resource.Success->{
                    hideProgressBar()
